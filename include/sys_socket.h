 ...
 */
#if defined (_WIN64)
typedef unsigned __int64 SYS_SOCKET; /**< Socket's handle */
#elif defined (_WIN32)
typedef unsigned int SYS_SOCKET; /**< Socket's handle */
#else
typedef int SYS_SOCKET;          /**< Socket's handle */
#endif

#define SYS_SOCKET_INVALID ((SYS_SOCKET)-1)
 ...
 * address.
 */
typedef unsigned long SYS_IPADDR;

#define SYS_IPADDR_ANY   ((SYS_IPADDR)0)
#define SYS_IPADDR_LOCAL ((SYS_IPADDR)0x7f000001UL) /* 127.0.0.1 */
 ...
 * @return             Completion status of operation: 0 (SYS_OK) = success,
 *                     negative return value is error.
 */
EXTERN int sys_SocketAccept (SYS_SOCKET socket, SYS_SOCKET *pNewSocket, 
 ...
 *                     negative return value is error.
 */
EXTERN int sys_SocketAddrToStr (SYS_IPADDR ipAddr, char* pbuf, int bufsize);
 ...
 *                     negative return value is error.
 */
EXTERN int sys_SocketBind (SYS_SOCKET socket, SYS_IPADDR addr, int port);
 ...
 *                     negative return value is error.
 */
EXTERN int sys_SocketClose (SYS_SOCKET socket);
 ...
 *                     negative return value is error.
 */
EXTERN int sys_SocketConnect (SYS_SOCKET socket, const char* host, int port);
 ...
 *                     negative return value is error.
 */
EXTERN int sys_SocketCreate (SYS_SOCKET* psocket);
 ...
 *                     negative return value is error.
 */
EXTERN int sys_SocketCreateUDP (SYS_SOCKET* psocket);
 ...
 *                     negative return value is error.
 */
EXTERN int sys_SocketsInit (void);
 ...
 *                     negative return value is error.
 */
EXTERN int sys_SocketsCleanup (void);
 ...
 * @return              Completion status of operation: 0 (SYS_OK) =
 *                      success, negative return value is error.
 */
EXTERN int sys_SocketListen (SYS_SOCKET socket, int maxConnection);
 ...
 */
EXTERN int sys_SocketRecvPeek
   (SYS_SOCKET socket, SYSOCTET* pbuf, SYSUINT bufsize);
 ...
 *                     received. Otherwise, the negative value is error code.
 */
EXTERN int sys_SocketRecv (SYS_SOCKET socket, SYSOCTET* pbuf, 
                           SYSUINT bufsize);
 ...
 *
 * @param socket       The socket's handle created by call to sys_SocketCreate
 *                     
 * @param pbuf         Pointer to the buffer for the incoming data.
 * @param bufsize      Length of the buffer.
 ...
 */
EXTERN int sys_SocketRecvFrom (SYS_SOCKET socket, SYSOCTET* pbuf,
			     SYSUINT bufsize, char * remotehost, 
                             SYSUINT hostBufLen, int * remoteport);
/**
 ...
 *                     negative return value is error.
 */
EXTERN int sys_SocketSend (SYS_SOCKET socket, const SYSOCTET* pdata, 
                           SYSUINT size);
 ...
 *                     negative return value is error.
 */
EXTERN int sys_SocketSendTo(SYS_SOCKET socket, const SYSOCTET* pdata, 
                            SYSUINT size, const char* remotehost,
                            int remoteport);
 ...
 *                     negative return value is error.
 */                                                       
EXTERN int sys_SocketSelect(int nfds, fd_set *readfds, fd_set *writefds, 
 ...
 *                     negative return value is error.
 */
EXTERN int sys_SocketStrToAddr (const char* pIPAddrStr, SYS_IPADDR* pIPAddr);
 ...
 *                     negative return value is error.
 */
EXTERN int sys_SocketConvertIpToNwAddr(char *inetIp, char *netIp);
 ...
 *                   negative return value is error.
 */
EXTERN int sysGetLocalIPAddress(char * pIPAddrs);


EXTERN int sys_SocketGetSockName(SYS_SOCKET socket, struct sockaddr_in *name, 
                                                      int *size);


EXTERN long sys_SocketHTONL(long val);

EXTERN short sys_SocketHTONS(short val);
 ...
 *
 * @return        SYS_OK, on success; -ve on failed.
 */
EXTERN int sys_SocketGetIpAndPort(SYS_SOCKET socket, char *ip, int len, int *port);
